...
| 画面 | 内容 | 状態 |
|---|---|---|
| `stocks.html`(在庫) | 食材ごとの在庫数量を検索・編集。未検索時は数量0の在庫を非表示にし、検索時のみ数量0を末尾に含めて表示する。検索フィールドにはクリアボタンあり(他画面と統一)。検索中のみ「新規食材を追加」ボタンが現れ、検索語を初期値にその場で食材を新規作成できる。更新日時は相対表記(さっき/N時間前/N日前/かなり前)。数量入力欄は更新ボタンを持たず、2秒間隔で変更を検出して自動保存する(ページ離脱時にも`beforeunload`で直近の変更を保存する) | 現行 |
| `recipes.html`(レシピ) | レシピの縦積みカードリスト表示・検索・食材による絞り込み・詳細表示・作成/編集(材料・調味料・手順・画像)。編集時の材料・調味料選択は`<select>`ではなく検索パネル形式のポップオーバーダイアログ(単一選択、その場での新規食材/調味料作成も可能)。詳細から「未定に追加」で日付未定の献立を1件作成できる。`#recipe-dialog`は常時全画面表示(調理中のスマートフォン操作性向上のため、`fullscreen-dialog`クラスをCSSで全画面化。URL遷移は行わない)。表示モードのヘッダーはタイトル+編集アイコンボタン、フッターは「戻る・閉じる(×)・進む」の3ボタンのみで、削除ボタンは編集モードのアクション行(キャンセル/保存と並び)に移設済み。戻る/進むは、ダイアログを開いた時点の検索結果・絞り込み条件のスナップショット(`buildRecipeDialogNav()`)内でのみ移動でき、先頭/末尾では該当ボタンが`disabled`になる | 現行 |
| `plans.html`(献立) | 日付ごとの献立(レシピ+人数、またはメモ行)を検討期間分のタイムラインと未定エリア・毎日エリアに表示し追加/編集。未定エリアの次に毎日エリア(検討期間内は毎日消費するとみなす献立)を表示し、両エリアとも検討期間によらず全件表示・日付欄なしで扱われる。パネル本体をクリックすると`#plan-dialog`が表示モードで開く(`recipes.html`の`#recipe-dialog`と同様、1つの`<dialog>`が表示/編集モードを切り替えるhead/body/foot構成)。表示モードは画像・参考URL・食材/調味料リスト(その献立自身の人数・個別オーバーライドを反映して算出)・手順、またはメモ内容を表示し、ヘッダーの編集アイコンボタンで同一ダイアログが編集モードに切り替わる(遷移先はレシピ紐づけ・メモ行どちらも既存の`showPlanEdit`のまま)。編集モードの日付`<select>`は「未定」「毎日」+検討期間内の日付から選び、選択値がそのまま献立の`date`/`type`の組に対応する。パネルのハンドルをPointerEventでドラッグして別日付・未定エリア・毎日エリアへ移動できる(ハンドル・編集/削除ボタンのクリックはパネル本体のクリックへ伝播させない)。レシピ紐づけの献立を表示中のみ`#plan-dialog`に`fullscreen-dialog`クラスが動的付与され全画面化する(メモ行は対象外で従来通り中央ダイアログサイズのまま)。フッターは「戻る・閉じる(×)・進む」の3ボタンのみで、削除ボタンは編集モードのアクション行に移設済み。戻る/進むは同日・同区分(`date`+`mealTime`+`type`が一致する、レシピ紐づけの献立のみ)内でのみ移動でき(`buildPlanDialogNav()`)、先頭/末尾やメモ行表示時は該当ボタンが`disabled`になる | 現行 |
...
- **マイグレーションは追記のみ**: 既存ファイルの内容変更は禁止。テーブル再構築が必要な場合も、一時テーブルへの退避→復元手順を新規マイグレーションとして追加する。
