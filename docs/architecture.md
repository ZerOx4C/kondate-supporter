...
| 画面 | 内容 | 状態 |
|---|---|---|
| `stocks.html`(在庫) | 食材ごとの在庫数量を検索・編集。未検索時は数量0の在庫を非表示にし、検索時のみ数量0を末尾に含めて表示する。検索フィールドにはクリアボタンあり(他画面と統一)。検索中のみ「新規食材を追加」ボタンが現れ、検索語を初期値にその場で食材を新規作成できる。更新日時は相対表記(さっき/N時間前/N日前/かなり前)。数量入力欄は更新ボタンを持たず、10秒間隔で変更を検出して自動保存する(ページ離脱時にも`beforeunload`で直近の変更を保存する) | 現行 |
| `recipes.html`(レシピ) | レシピの縦積みカードリスト表示・検索・食材による絞り込み・詳細表示・作成/編集(材料・調味料・手順・画像)。編集時の材料・調味料選択は`<select>`ではなく検索パネル形式のポップオーバーダイアログ(単一選択、その場での新規食材/調味料作成も可能)。詳細から「未定に追加」で日付未定の献立を1件作成できる | 現行 |
