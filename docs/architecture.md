...
| `ingredients.html`(食材マスタ 旧) | 食材マスタの旧CRUD画面(インラインフォーム形式) | **孤立(未使用)**。masters.html統合後もどこからもリンクされず残存 |
| `seasonings.html`(調味料マスタ 旧) | 調味料マスタの旧CRUD画面(インラインフォーム形式) | **孤立(未使用)**。同上 |

> 孤立ページの `ingredients.html`/`seasonings.html` は誤って参照・復活させないよう注意。削除するかどうかはROADMAP.md側の判断とする。
